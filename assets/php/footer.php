<footer class='intro-50'>
    <div class='content-bo div-sh-2 bg-smoky-black clouds-600 px-4 py-5'>
        <div class="row">
            <div class="offset-md-2 col-md-8">
                <a href='https://www.instagram.com/hmeitb/' class='hvr-grow-rotate' target='_blank'><img src='assets/images/hme.jpg' style='width:58.5px;height:70.75px'><a>
                <h2 class='merriweather-700 loose-2 mt-2'>Himpunan Mahasiswa Elektroteknik<br>Institut Teknologi Bandung</h2>
                <hr class='border-alizarin-700' style='width:30%;'>

                <div class="row">
                    <div class="col-md-6 text-left pt-4">
                        <h3 class='montserrat impulse-yellow-100 fw-600'><i>Contact Person</i></h3>
                        <p class='geomanist'>Achmad Fajar / 0877-7148-0483<br>
                           Firdaus Wahyu / 0812-1556-5256</p>
                    </div>
                    <div class="col-md-6 text-left pt-4">
                        <h3 class='montserrat impulse-yellow-100 fw-600'><i>Alamat</i></h3>
                        <p class='geomanist'>HME ITB Basement Labtek 8, ITB; Jl. Ganesha No.10, Coblong, Kota Bandung, Jawa Barat 40132</p>
                    </div>
                </div>
                
                <div class="row pt-5">
                    <div class="col-5 text-left">
                        <a href='#home' class='smalltext alizarin montserrat-700 loose-2'>Back to Top <i class="fas fa-arrow-up"></i></a>
                    </div>
                    <div class="col-7 text-right">
                        <a class='smalltext clouds-900 montserrat-700 loose-2'>&copy; Copyright HME ITB 2018</a>
                    </div>
                </div>
                
            </div>
        </div>
        
        
    </div>
</footer>